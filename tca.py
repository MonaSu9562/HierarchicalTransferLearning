...
"""
    ...
    Original author: Jindong Wang
	Modified by: Mengyuan Su
...
class TCA:
    ...
    def fit_transform(self, x_src, x_tar, x_tar_o=None):
        ...
        Kc = np.dot(np.dot(np.dot(np.linalg.pinv(forPinv), K), H), K)#pinv 广义逆矩阵 pseudo-inverse of a matrix
        Kc[np.isnan(Kc)] = 0

        D, V = np.linalg.eig(Kc) 
        #D, V = eigs(Kc) # 求特征值 D 特征值eigenvalues V 特征向量normalized (unit “length”) eigenvectors
        #原本是numpy.linalg.eig 改成了scipy.sparse.linalg.eigs
